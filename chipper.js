...
var chipt={
  ...
  bar:function(s){
    chipt.mel.forEach((x,i)=>{
      ...
      if(i%4==0)chipt.kick(q); // percusion
      if(i==4||i==12)chipt.noise(q,.12,.12,2500); // percusion
      if(i%2)chipt.noise(q,.045,.045,7000); // percusion
    });
  },
  ...
  start:function(){
    ...
    (function loop(){
      if(!run)return;
      for(let i=0;i<3;i++)chipt.bar(t+i*16*chipt.dt);
        t+=48*chipt.dt;
      chipt.pass++;
